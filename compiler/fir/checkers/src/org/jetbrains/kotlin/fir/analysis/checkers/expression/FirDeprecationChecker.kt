...
object FirDeprecationChecker : FirBasicExpressionChecker() {
    ...
    private fun FirStatement.isDelegatedPropertySelfAccess(context: CheckerContext, referencedSymbol: FirBasedSymbol<*>): Boolean {
        return source?.kind == KtFakeSourceElementKind.DelegatedPropertyAccessor &&
                (context.containingDeclarations.getOrNull(context.containingDeclarations.lastIndex - 1) == referencedSymbol.fir ||
                        context.containingDeclarations.lastOrNull() == referencedSymbol.fir)
    }
