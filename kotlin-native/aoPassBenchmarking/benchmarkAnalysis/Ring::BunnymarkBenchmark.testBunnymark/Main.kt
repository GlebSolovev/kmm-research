...
open class BunnymarkBenchmark {
    ...
    fun testBunnymark() {
        ...
        // val random = Random() // MODIFIED: use benchmarks' Random instead of shared one
        ...
        fun executeFrame() {
            bunnys.fastForEach { bunny ->
                ...
                if (bunny.y > maxY) {
                    bunny.speedYf *= -0.85f
                    bunny.y = maxY
                    bunny.radiansf = (Random.nextFloat() - 0.5f) * 0.2f // MODIFIED: replace random with Random
                    if (Random.nextFloat() > 0.5) { // MODIFIED: replace random with Random
                        bunny.speedYf -= Random.nextFloat() * 6 // MODIFIED: replace random with Random
                    }
...
class BunnyContainer(maxSize: Int) : FSprites(maxSize) {
    ...
    var FSprite.speedXf: Float get() = speeds[index * 2 + 0]
        set(value) { speeds[index * 2 + 0] = value }
    var FSprite.speedYf: Float get() = speeds[index * 2 + 1]
        set(value) { speeds[index * 2 + 1] = value }
}
...
open class FSprites(val maxSize: Int) {
    ...
    var FSprite.x: Float get() = f32[offset + 0]
        set(value) { f32[offset + 0] = value }
    var FSprite.y: Float get() = f32[offset + 1]
        set(value) { f32[offset + 1] = value }
    var FSprite.radiansf: Float get() = f32[offset + 4]
        set(value) { f32[offset + 4] = value }
...
fun main() {
    BunnymarkBenchmark().testBunnymark()
}
...
class Random constructor() {
    ...
    companion object {
        ...
        fun nextFloat(): Float = nextDouble(67108864.0).toFloat() // MODIFIED: replace nextFloat() function from shared Random with this
    }
}
