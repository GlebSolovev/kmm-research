 ...
 */
public fun interface SoftValueCleaner<V> {
    public fun cleanUp(value: V?)
}
 ...
 */
public class CleanableSoftValueCache<K : Any, V : Any>(
    ...
    public fun get(key: K): V? = backingMap[key]?.get()

    public fun computeIfAbsent(key: K, f: (K) -> V): V {
     ...
     */
    public val keys: Set<K> get() = backingMap.keys.toSet()
...
private class SoftReferenceWithCleanup<K, V>(
    ...
    // The soft reference won't be used as a key, so it's OK to have a non-matching hash code implementation.
